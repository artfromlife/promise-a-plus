...
function resolvePromise(promise, x, resolve, reject) {
  // promise 就是then 方法要返回的 promise , x 是你的then 回调执行的结果
  // 如果 promise 和 x 指向同一对象，以 TypeError 为据因拒绝执行 promise
  // 这是为了防止死循环
  if (promise === x) { // 讲真，为啥会是自己啊，我不理解， 反正最后你这个then 方法里的promise 是要resolve x 的 ， 就看x 是什么
  ...
  // 如果 x 为对象或者函数
    ...
    // 如果 x 不为对象或者函数，以 x 为参数执行 promise ， x 不是 对象， 也不是函数， 也不是 Promise 那就赶紧resolve 啊， 让他fulfilled , 让注册的事件赶紧进入待执行队列！
    resolve(x);
  }
}
